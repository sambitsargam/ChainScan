...
function Appy() {
  ...
  useEffect(() => { // Only run once when the component mounts
    ...
    const fetchTransactions = async () => {
      const add = address;
      console.log(address);
      const response = await fetch(`https://api-sepolia.etherscan.io/api?module=account&action=txlist&address=${add}&apikey=KITA7GXA7YZJQ4VH4H4DE4A91E22HMQMB3`);
      const data = await response.json();
      console.log(data);
      setTransactions(data.result.slice(-10).reverse());
    };
        ...
        <Tbody>
          {transactions.map((tx) => (
            <Tr key={tx.hash}>
              ...
              <Td>{ConvertValue(tx.value)} ETH</Td>
            </Tr>
          ))}
        </Tbody>
      </Table>
